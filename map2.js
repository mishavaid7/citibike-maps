  ...
  map2.addLayer(
    {
      ...
      source: {
        type: "geojson",
        data: "data/sept2020Stops.geojson",
      },
      paint: {
        'circle-radius': 4,
        // "circle-color": '#26547C',
...
// Create the popup
map2.on('click', 'stop-stops', function (e) {
  var stationName = e.features[0].properties.end_station_name;
  var tripCount = e.features[0].properties.tripCount.toLocaleString();
  
  new mapboxgl.Popup()
      .setLngLat(e.lngLat)
      .setHTML('<h3>' + tripCount + ' trips ended at ' + stationName + '</h3>'
          )
      .addTo(map2);
  });
  map2.on('mouseenter', 'stop-stops', function () {
  map2.getCanvas().style.cursor = 'pointer';
  });
  map2.on('mouseleave', 'stop-stops', function () {
